...
get_number.onclick = async function() {
  await contract.methods.getNumberOfProducts().call().then( function( info ) {
    console.log("info: ", info)
    get_number.innerHTML = 'Number of products: ' + info
  });
}

// Farmer information initialization
submit_initialization.onclick = async function() {
  farmer_name = $("#farmer_name").val()
  farmer_age = $("#farmer_age").val()
  await contract.methods.initialization(farmer_name, Number(farmer_age)).send({from: account }).then(function(tx) {
      console.log(tx)
...
submit_creator.onclick = async function() {
  ...
  await contract.methods.createNewProduct(product_name_creator, Number(number_of_product_creator), place_creator, time_creator).send({from: account }).then(function(tx) {
    console.log(tx)
...
submit_editor.onclick = async function() {
  ...
  await contract.methods.editProductInformation(Number(product_id_editor), product_name_editor, Number(number_of_product_editor), place_editor, time_editor).send({from: account }).then(function(tx) {
    console.log(tx)
