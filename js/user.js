...
const submit_product_number_check = $("#submit_product_number_check")[0]
const submit_get_product_id_check = $("#submit_get_product_id_check")[0]
...
submit_product_number_check.onclick = async function() {
  await contract.methods.getNumberOfProducts($("#product_farmer_id").val()).call().then(function(info) {
    console.log("info: ", info)
    $("#product_number_return")[0].innerHTML = "Number of products: " + info
  });
}

// Get all the product ID of the farmer
submit_get_product_id_check.onclick = async function() {
  await contract.methods.getMyProductId($("#get_product_farmer_id").val()).call().then(function(info) {
    console.log("info: ", info)
    $("#get_product_id_return")[0].innerHTML = "All products: " + info
  });
}
